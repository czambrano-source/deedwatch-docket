...
const N8N_URL = "https://n8n.duppla.co/webhook/delete-predial-alejandria";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!body?.id_inmueble || !body?.tipo_doc) {
      return new Response(
        JSON.stringify({ error: "Campos requeridos: id_inmueble, tipo_doc" }),
    ...
    return new Response(
      JSON.stringify({ success: true, message: "Documento eliminado de Alejandria" }),
